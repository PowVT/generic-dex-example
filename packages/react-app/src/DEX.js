...
export default function DEX(props) {
  ...
  const rowForm = (title,icon,onClick)=>{
    return (
      <Row>
        <Col span={8} style={{textAlign:"right",opacity:0.333,paddingRight:6,fontSize:19}}>{title}</Col>
        <Col span={16}>
  ...
  if(props.readContracts && props.readContracts[contractName]){
    ...
    display.push(
      <div>
        ...
        {rowForm("deposit ETH/ADP","📥",async (value)=>{
          let valueInEther = ethers.utils.parseEther(""+value)
        ...
        {rowForm("withdraw ETH/ADP","📤",async (value)=>{
          let valueInEther = ethers.utils.parseEther(""+value)
  ...
  return (
    <div>
      <div style={{position:"fixed",right:400,top:50,padding:10}}>
        <Curve
        ...
        title={(
          <div>
            ...
            <div style={{float:'right',fontSize:24}}>
              ...
              <TokenBalance name={tokenName} img={"ADP"} address={contractAddress} contracts={props.readContracts} />
